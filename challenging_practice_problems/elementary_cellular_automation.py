...
def binarize_seed(seed, seed_list):
    x = 128
    while x >= 1:
        if seed >= x:
            seed -= x
...
def cellular_automaton(origin, seed, n):
    seed_list = []
    binarize_seed(seed, seed_list)
    count = 0
    while count < n:
        next = ''
        for i in range(0, len(origin)):
            now = origin[i-1] + origin[i] + origin[(i+1) % len(origin)]
            if num_dict[now] in seed_list:
                next += 'x'
            else:
                next += '.'
        origin = next

...
def exclude_reciprocal(graph, exclude, u, v, n, k):
    if n > k:
        return
    if u not in exclude:
        exclude[u] = []
    for x in graph[u]:
        if x == v:
            exclude[u].append(x)
        exclude_reciprocal(graph, exclude, x, v, n+1, k)


def compute_ranks(graph, k):
    exclude = {}
    for page in graph:
        exclude_reciprocal(graph, exclude, page, page, 0, k)
    ...
    for i in range(0, numloops):
        ...
        for page in graph:
            newrank = (1 - d) / npages
            for node in graph:
                if page in graph[node] and page not in exclude[node]:
